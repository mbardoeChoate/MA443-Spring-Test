...
If the variables Race and DayBoarding are independent we would expect about the same percentage of each race to be day students. The percentage of total day students is 63/243
...
```{r}
table2 <- matrix( c( 17+8, 35+ 53, 22, 491), ncol = 2, byrow = TRUE)
colnames(table2) <- c( "Hepatitis C", "No Hepatitis C")
rownames(table2) <- c( "Tatoo", "None")
table2 <- as.table(table2)
table2
```

```{r}
chi.hep2 <- chisq.test( table2)
...
```{r}
table.sports.press <- table(pol$SportsLevelFall, pol$PressureGeneral)
table.sports.press
```
...
```{r}
table.sports.press.marg <- addmargins(table.sports.press)
table.sports.press.marg
```

3. State the null and alternative hypothesis for a chi square test of independence for these two variables.

*Answer here*
H0: the variables SportsLevelFall and PressureGeneral are independent
Ha: the variables SportsLevelFall and PressureGeneral are associated 


4. Run the chi square test.

```{r}
chi.sports.press <- chisq.test(table.sports.press)
chi.sports.press
```
