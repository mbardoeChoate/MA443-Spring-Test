...
mike.and.ike1 <- function( ){
  ...
  repeat{
    total.in.hand <- total.in.hand + 1
    
    flavor <- sample(1:5, 1)
    
    if( flavor == 1){ lemon <- lemon +1 }
    if( flavor == 2) {lime <- lime + 1 }
    
    if( total.in.hand == 4){break}
  }
  
  success <- "no"
  if( lemon >= 1 & lime >=1 ){ success <- "yes"}
  return(success)
...
```{r}
thousand.trials.mike <- replicate( 1000, mike.and.ike1() )
table(thousand.trials.mike)
```
...
```{r}
mike.and.ike2 <- function( ){
  
  lemon.have <- 0
  lime.have <- 0
  
  
handful <- sample( 1:5, 4, replace= TRUE)
  
i <- 1

repeat{
if( handful[i] == 1){ lemon.have <- lemon.have +1 }
if( handful[i] == 2){ lime.have <-lime.have + 1 }
...
success <- "no"
  if( lemon.have >= 1 & lime.have >=1 ){ success <- "yes"}
  return(success)
...
```{r}
thousand.trials.mike <- replicate( 1000, mike.and.ike2() )
table(thousand.trials.mike)
```
...
mike.and.ike.3 <- function(){
  ...
  if( sum(handful == 1) >0 ){ lemon <- TRUE}
  if( sum(handful == 2) >0 ){ lime <- TRUE}
  
  
  if( lemon == TRUE & lime == TRUE){return( "success!") }
...
```{r}
thousand.mike3 <- replicate( 1000, mike.and.ike.3() )

table(thousand.mike3)
```
...
jersey <- function(){
  ...
  jersey.number <- sample(1: (10-sprints)  , 1)
  if( jersey.number == 1) {break}
  sprints <- sprints + 1
  }
...
```{r}
thousand.jersey <- replicate(1000, jersey() )
table( thousand.jersey )
```
