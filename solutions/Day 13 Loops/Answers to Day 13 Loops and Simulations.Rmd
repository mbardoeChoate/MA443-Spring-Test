...
```{r}
did.underdog.win( 4, 60)
```



```{r}
thousand.trials.underdog <- replicate( 1000, did.underdog.win( 50, 60) )
...
```{r}
how.many.kids <- function( num.males, num.females){
  ...
  repeat{
  gender.num <- sample(1:100, 1)
  if( gender.num <= 50){ males <- males +1 }
  if( gender.num > 50){ females <- females + 1 }
  
  if( males >= 1 & females >= 1){ break}
  }
