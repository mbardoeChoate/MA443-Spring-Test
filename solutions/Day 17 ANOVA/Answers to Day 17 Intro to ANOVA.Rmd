---
title: "Day 17 Introduction to ANOVA"
...
We can see if this worked by remakeing the boxplot:
...
This says that there are probably some differences in the means of the stress levels among grades. 

But just as we did for regression, let's check to make sure something funny isn't going on with the residuals. 
...
```
We see that there are a few outliers in different groups. Which groups have the outliers?


We also want to check for a normal distribution of the scores. Let's do this with a qqplot. The ```aov``` command has already computed all the information for this. We just have to plot it.

```{r}
plot(anov1,2)
```

Looks pretty good, except for some variation at the bottom. 
...
```{r}
boxplot(choc$Rating~choc$Bean_Type,
        col = 2:4)
```
...
```{r}
tapply(choc$Rating , choc$Bean_Type , FUN = mean)
...
```{r}
anova2 <- aov(choc$Rating~choc$Bean_Type )
summary(anova2)
```
