...
solitare.fun <- function(){
  ...
  repeat{
    ...
    number.of.games <- number.of.games + 1
    
    game.result <- sample( 1:100, 1)
    
    if( game.result <= 80){ number.of.wins <- number.of.wins + 1}
    
    if( number.of.wins == 5){break}
...
```{r}
thousand.solitare <- replicate(1000, solitare.fun() )
mean(thousand.solitare)
```
...
```{r}
covid <- function( percent.have.covid, number.of.students ){
  ...
  repeat{
    ...
    if( covid.test <= percent.have.covid){ have.covid <- have.covid +1 }
    
    if( student.number == number.of.students){ break}
  }
  return( have.covid)
}
```
